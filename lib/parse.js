...
var getValue = require('./klineValue').getValue();
...
var saveListToRedis = function(key, values, redis, suffix cb) {
		values.forEachSync(function(item, index, cback) {
			var value = item.length > 1 ? item.join('|') : item[0];
...
var doList = function(key, values, redis, cb) {
		var suffix = key.substring(parseFloat(key.lastIndexOf('.')) + 1); //WK MTH HY FY SY 05M 15M 30M 60M 
			...
			if(temp_task.parse['interface'] != undefined) {
				...
				redisClient_array.forEachSync(function(redis, index, cback) {
					...
					function_array, function() {
						self.emit('fetch-finished', temp_task);
					});
				});
			}
		});
