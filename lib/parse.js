...
/*
 * 如果item是数组，则转换成以|间隔的字符串
 */
var getValue = function(item) {
		return item.length > 1 ? item.join('|') : item[0];
	};
/*
 *处理日K 周K 月K 半年K 季K 年K
 */
var saveNoMinuteListToRedis = function(key, values, redis,suffix cb) {
		values.forEachSync(function(item, index, cback) {
			...
			redis.lindex(key, -1, function(e, r) {
				if(e) {
					...
					if(check[suffix](time, time_redis)) { //在同一周则更新
						redis.lset(key, -1, value, function(err, repl) {
							if(err) {
								error(err);
							}
							cback();
...
var KEYS_STR = 'DAY|WK|MTH|HY|FY|SY';
var doList = function(key, values, redis, cb) {
		var suffix = key.substring(parseFloat(key.lastIndexOf('.')) + 1); //WK MTH HY FY SY
		if(KEYS_STR.indexOf(suffix) != -1) {
			saveNoMinuteListToRedis(key, values, redis, suffix, cb);
		} else if(suffix.endWith() {
			saveMinuteListToRedis(key, values, redis, suffix, cb);
		}
